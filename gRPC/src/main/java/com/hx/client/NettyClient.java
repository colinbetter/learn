...
public class NettyClient {
    ...
    public NettyClient(String host, int port) {
        futureStubChannel = NettyChannelBuilder.forAddress(host, port).directExecutor()
                .usePlaintext(true)
                .build();
        futureStub = SimpleCalculatorGrpc.newFutureStub(futureStubChannel);//newBlockingStub();
        blockingStubChannel = NettyChannelBuilder.forAddress(host, port).directExecutor()
                .usePlaintext(true)
                .build();
        blockingStub = SimpleCalculatorGrpc.newBlockingStub(blockingStubChannel);
        stubChannel = NettyChannelBuilder.forAddress(host, port).directExecutor()
                .usePlaintext(true)
                .build();
    ...
    public static void main(String[] args) throws InterruptedException {
        NettyClient client = new NettyClient("127.0.0.1", 50051);
        for (int i = 0; i < 5; i++) {
