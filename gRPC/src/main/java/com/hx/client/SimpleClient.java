...
public class SimpleClient {
    ...
    public SimpleClient(String host, int port) {
        futureStubChannel = ManagedChannelBuilder.forAddress(host, port).directExecutor()
                .usePlaintext(true)
                .build();
        futureStub = SimpleCalculatorGrpc.newFutureStub(futureStubChannel);//newBlockingStub();
        blockingStubChannel = ManagedChannelBuilder.forAddress(host, port).directExecutor()
                .usePlaintext(true)
                .build();
        blockingStub = SimpleCalculatorGrpc.newBlockingStub(blockingStubChannel);
        stubChannel = ManagedChannelBuilder.forAddress(host, port).directExecutor()
                .usePlaintext(true)
                .build();
